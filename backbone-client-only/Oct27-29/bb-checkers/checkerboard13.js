...
var Router = Backbone.Router.extend({
  ...
  checkers: function (num) {
    if (!num) num=0;
    var games = this.checkerGames;
    if (!games[num]) games[num]=new Checkers();
    this.switchTo(games[num]);
  },
  chess: function (num) {
    if (!num) num=0;
    var games= this.chessGames;
    if (!games[num]) games[num]=new Chessmen();
    this.switchTo(games[num]);
  }
});
